        ...
        if checkpoint:
            ...
            if weights:
                if save_condition == 'best':
                    save_path = os.path.join(PATH, 'Microbleednet_model_weights_bestdice.pth')
                    ...
                    if (epoch % N) == 0:
                        save_path = os.path.join(PATH,
                                                 'Microbleednet_model_weights_epoch' + str(epoch) + '.pth')
                        torch.save(model.state_dict(), save_path)
                elif save_condition == 'last':
                    save_path = os.path.join(PATH, 'Microbleednet_model_weights_beforeES.pth')
                    torch.save(model.state_dict(), save_path)
                else:
                    raise ValueError("Invalid saving condition provided! Valid options: best, everyN, last")
            else:
                if save_condition == 'best':
                    save_path = os.path.join(PATH, 'Microbleednet_model_bestdice.pth')
                    if val_acc > best_val_acc:
                        torch.save({
                            'epoch': epoch,
                    ...
                    N = tr_prms['EveryN']
                    if (epoch % N) == 0:
                        save_path = os.path.join(PATH, 'Microbleednet_model_epoch' + str(epoch) + '.pth')
                        torch.save({
                            'epoch': epoch,
                            'model_state_dict': model.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(),
                    ...
                    save_path = os.path.join(PATH, 'Microbleednet_model_beforeES.pth')
                    torch.save({
                        'epoch': epoch,
                        'model_state_dict': model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
